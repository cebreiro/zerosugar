...
void NavMeshTesterTool::init(zerosugar::xr::navi::RecastDemoVisualizerInterface* visualizer)
{
    m_visualizer = visualizer;
    m_navMesh = &visualizer->GetNavMesh();
    m_navQuery = &visualizer->GetNavMeshQuery();
    recalc();
    ...
    m_neighbourhoodRadius = visualizer->GetAgentRadius() * 20.0f;
    m_randomRadius = visualizer->GetAgentRadius() * 30.0f;
}
...
void NavMeshTesterTool::recalc()
{
	...
	else if (m_toolMode == TOOLMODE_FIND_POLYS_IN_SHAPE)
	{
		if (m_sposSet && m_startRef && m_eposSet)
		{
			...
			const float agentHeight = m_visualizer ? m_visualizer->GetAgentHeight() : 0;
...
{
	duDebugDraw& dd = m_visualizer->GetDebugDraw();
	...
	const float agentRadius = m_visualizer->GetAgentRadius();
	const float agentHeight = m_visualizer->GetAgentHeight();
	const float agentClimb = m_visualizer->GetAgentClimb();
