...
{
    class CancelDungeonMatchHandler final : public IGamePacketHandlerT<network::game::cs::CancelDungeonMatch>
    {
    public:
        auto HandlePacket(GameServer& server, Session& session,
            UniquePtrNotNull<network::game::cs::CancelDungeonMatch> packet) -> Future<void> override;
    };
}
