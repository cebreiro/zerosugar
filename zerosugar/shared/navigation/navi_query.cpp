...
namespace zerosugar::navi
{
        ...
        -> std::pair<dtPolyRef, Vector>
    {
        ...
        query.findNearestPoly(center.GetData(), extent.GetData(), &filter, &result.first, result.second.GetData());
    ...
    {
        const auto [startPoly, startNearestPoint] = FindNearestPoly(query, start, extent, filter);
        if (!startPoly)
        {
            return {};
        }

        const auto [endPoly, endNearestPoint] = FindNearestPoly(query, end, extent, filter);
        if (!endPoly)
        {
            return {};
        }
